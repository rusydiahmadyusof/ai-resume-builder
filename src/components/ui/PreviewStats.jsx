function PreviewStats({ resumeData, generatedContent }) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-4">
      <h3 className="text-sm font-semibold text-gray-900 mb-3">Resume Statistics</h3>
      <div className="space-y-2 text-sm">
        <div className="flex justify-between">
          <span className="text-gray-600">Work Experience:</span>
          <span className="font-medium text-gray-900">{stats.workExperience}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-600">Education:</span>
          <span className="font-medium text-gray-900">{stats.education}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-600">Skills:</span>
          <span className="font-medium text-gray-900">{stats.skills}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-600">Certifications:</span>
          <span className="font-medium text-gray-900">{stats.certifications}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-gray-600">Languages:</span>
          <span className="font-medium text-gray-900">{stats.languages}</span>
        </div>
        ...
        <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-200">
          {stats.hasPhoto && (
            <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded">Photo</span>
          )}
          {stats.hasSummary && (
            <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded">Summary</span>
          )}
          {stats.hasGeneratedContent && (
            <span className="text-xs bg-purple-100 text-purple-700 px-2 py-1 rounded">AI Generated</span>
          )}
        </div>
      </div>
    </div>
